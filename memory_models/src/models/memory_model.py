...
from memory_models.src.features.feature_engineering import preprocess_features

class MemoryModel:
    """
    XGBoost model for binary classification of memory logs (normal vs. anomalous).
    """
    
    def __init__(self, model_path=None, cmd_encoding_map_path=None):
        """Initialize the memory model with the model path."""
        # Get the absolute path to the model file
        if model_path is None:
            # First, try finding the model in the models subdirectory
            self.model_path = os.path.join(root_dir, "models", "memory_model.pkl")
            
            # If not found, try using the one in the root directory
            if not os.path.exists(self.model_path):
                self.model_path = os.path.join(root_dir, "..", "xgb_memory_model.pkl")
        else:
            self.model_path = model_path
            
        print(f"Using memory model: {self.model_path}")
    ...
    def predict(self, df):
        """
        Predict if memory logs are anomalous (1) or normal (0).
        
        Args:
            df (pandas.DataFrame): Memory log data
    ...
    def predict_proba(self, df):
        """
        Get probability estimates for memory logs being anomalous.
