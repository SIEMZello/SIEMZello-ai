...
# Import model class
from disk_models.src.models.disk_model import DiskModel

class DiskAnalyzer:
    """
    A wrapper class for disk log anomaly detection.
    """
    
    def __init__(self, model_path=None, cmd_encoding_map_path=None):
        """
        Initialize the analyzer with the disk model.
        
        Args:
            model_path (str, optional): Path to disk model file
            cmd_encoding_map_path (str, optional): Path to CMD encoding map file
        """
        print("Initializing DiskAnalyzer...")
        self.disk_model = DiskModel(model_path=model_path, cmd_encoding_map_path=cmd_encoding_map_path)
        print("Model loaded successfully.")
    
    def analyze(self, disk_data):
        """
        Analyze disk logs for anomalies.
        
        Args:
            disk_data (pd.DataFrame): Disk log data
        ...
        # Binary Detection
        print(f"Analyzing {len(disk_data)} disk records...")
        anomaly_predictions = self.disk_model.predict(disk_data)
        anomaly_probabilities = self.disk_model.predict_proba(disk_data)
        ...
        for i, (is_anomaly, probability) in enumerate(zip(anomaly_predictions, anomaly_probabilities)):
            ...
            # Add the original disk details for context
            important_cols = ['ts', 'CMD', 'RDDSK', 'WRDSK', 'WCANCL', 'DSK'] 
            for col in important_cols:
                if col in disk_data.columns:
                    result[col] = str(disk_data.iloc[i][col])
    ...
    def export_results(self, results, filename="disk_analysis_results.json"):
        """
...
def generate_sample_data(num_samples=5):
    """
    Generate sample disk data for testing.
    
    Args:
        num_samples (int): Number of sample records to generate
        
    Returns:
        pd.DataFrame: Sample disk data
    """
    import numpy as np
    import pandas as pd
